<template>
  ...
  <section id="home" class="container text-white">
    <div class="p-4 mb-8 relative">
      <input type="text" placeholder="Search for a city or state" v-model="search" ref="searchQuery" @input="getSearchQueryResult"
      ...
      <ul class="absolute bg-weather-secondary text-white w-full shadow-md py-2 px-1 top-[66px]" v-if="mapboxSearchResults">
        ...
        <p class="px-1 text-warning" v-if="!searchError && mapboxSearchResults.length === 0">No result match your query, try a
          different term</p>
        <template v-else>
